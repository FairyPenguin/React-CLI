    ...
    try {
        ...
        if (choices.generalChocies.nextjsRouteOrRegularRecactComponent === "Nextjs Route") {
            ...
            if (choices.nextjsChoices) {
                ...
                fs.writeFileSync(nextjsRouteFilePath, nextjsRouteFunctionKeywordOutput);

                //  nested components folder step
                if (choices.nextjsChoices.nextjsNestedComponentsFolder === "Yes") {

                    const nextRouteNestedComponentFolderPath = path.join(`${nextjsRouteFolderPath}/`, `${choices.nextjsChoices.nextjsRouteName}Components`);

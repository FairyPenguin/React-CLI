...
export default async function createFilesAndFolders(
  ...
  try {
    if (choices.regularComponentsChoices) {
      ...
      const regularReactComponentFolderPath = path.join(
        "./",
        `${choices.regularComponentsChoices.componentName}`
      );

      const regularReactComponentSingleFilePath = path.join(
        "./",
        `${choices.regularComponentsChoices.componentName}.${choices.generalChocies.extention}`
      );

      // React Component file path

      const reactComponentFilePath = path.join(
        `${regularReactComponentFolderPath}/`,
        `${choices.regularComponentsChoices.componentName}.${choices.generalChocies.extention}`
      );

      // React Component  CSS file path

      const reactComponentCSSFilePath = path.join(
        `${regularReactComponentFolderPath}/`,
        `${choices.regularComponentsChoices.componentName}.module.css`
      );

      const reactComponentSingleFileCSSFilePath = path.join(
        "./",
        `${choices.regularComponentsChoices.componentName}.module.css`
      );
      ...
      // Use mustache to render the data in the component file

      // Function template
      const reactComponentFunctionKeywordOutput = mustache.render(
        functionKeywordTemplate,
        reactComponentData
      );

      // Const template
      const reactComponentConstOutput = mustache.render(
        constTemplate,
        reactComponentData
      );
      ...
      if (
        ...
        if (
          choices.regularComponentsChoices.folderOrFile ===
          ...
          if (
            ...
            fs.writeFileSync(
              regularReactComponentSingleFilePath,
              reactComponentFunctionKeywordOutput
            );
          }
          ...
          if (choices.generalChocies.constOrFunctionKeyword === "Const") {
            fs.writeFileSync(
              regularReactComponentSingleFilePath,
              reactComponentConstOutput
            );
          }

          fs.writeFileSync(
            reactComponentSingleFileCSSFilePath,
            "/* CSS Module */"
          );
        }
        ...
        if (
          ...
          if (choices.generalChocies.constOrFunctionKeyword === "Const") {
            fs.writeFileSync(reactComponentFilePath, reactComponentConstOutput);
          }

          if (
            choices.regularComponentsChoices.nestedSubComponentsFolder === "Yes"
          ) {
            const regularNestedReactComponentFolderPath = path.join(
              `${regularReactComponentFolderPath}/`,
              `${choices.regularComponentsChoices.componentName}Components`
            );
